    ...
    public partial class PostPage : ContentPage
    {
        ...
        public PostPage(Item item)
        {
            ...
            try
            {
                ...
                throw (new Exception("Some crash error or other"));        // illustrate App Center tracking by throwing exception (inside try/catch)
        ...
        // bespoke method to send error tracking to App Center
        private async void TrackError(Exception exc, IDictionary<string, string> properties)
        {
            if (await Crashes.IsEnabledAsync())       // check first whether crash/error tracking is actually enabled
            {
